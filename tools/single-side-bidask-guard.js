...
export function normalizeForcedSingleSidedSolBidAskArgs(args = {}, options = {}) {
  ...
  if (normalized.strategy !== "bid_ask") {
    withRepair(repairs, "strategy", normalized.strategy, "bid_ask", "forced nanocap deploy strategy");
  ...
  if (deployAmountSol != null && deployAmountSol > 0) {
    if (suppliedAmount == null || suppliedAmount < deployAmountSol) {
      withRepair(
        ...
        suppliedAmount,
        deployAmountSol,
        "forced nanocap deploy uses the full configured deploy amount",
      );
  ...
  if (binsBelow != null && binsBelow > 0 && (normalized.bins_below == null || finiteNumber(normalized.bins_below) === 0)) {
    withRepair(repairs, "bins_below", normalized.bins_below, binsBelow, "forced nanocap deploy uses configured downside bins");
